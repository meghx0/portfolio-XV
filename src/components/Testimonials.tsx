      ...
      <div ref={ref} className="mx-auto max-w-6xl px-5">
        ...
        <div className="mt-12 grid gap-5 md:grid-cols-2 md:max-w-4xl md:mx-auto">
          {testimonials.map((t, i) => (
            <figure
              key={t.name}
              className={`reveal ${visible ? 'is-visible' : ''} flex flex-col border-2 border-ink-900 bg-cream-50 p-6 shadow-[4px_4px_0_0_#0a0a0a] transition-all hover:translate-x-[-2px] hover:translate-y-[-2px] hover:shadow-[6px_6px_0_0_#0a0a0a]`}
              style={{ transitionDelay: `${i * 100}ms` }}
            >
              <Quote className="h-8 w-8 text-ink-300" />
              ...
              <figcaption className="mt-5 flex items-center gap-3 border-t-2 border-ink-900 pt-4">
                <span className={`flex h-10 w-10 items-center justify-center border-2 border-ink-900 ${t.color} font-display text-sm font-bold text-ink-900`}>
                  {t.name.charAt(0)}
                </span>
                <div>
                  <div className="font-display text-sm font-bold text-ink-900">{t.name}</div>
                  <div className="font-mono text-[11px] font-bold text-ink-500">{t.role}</div>
                </div>
              </figcaption>
            </figure>
          ))}
